 ...
 */
pre_execution(150)
static void i2s_bus_pool_init(void)
{
 ...
 */
static struct i2s_bus_host* host_from_dev(struct device* dev)
{
    int i;
    for (i = 0; i < I2S_BUS_HOST_MAX; i++)
        if (osal_pool_is_used(&s_host_pool, i) && s_hosts[i].dev == dev)
            return &s_hosts[i];
    return NULL;
}
 ...
 */
static struct i2s_bus_client* client_from_dev(struct device* dev)
{
    int id = (int)board_dev_find(device_get_name(dev));
    if (id < 0 || id >= DEV_ID_COUNT || !s_clients[id].dev)
        return NULL;
    return &s_clients[id];
}
...
static int i2s_host_init_impl(struct device* pdev, const void* cfg)
{
    ...
    if (!pdev || !cfg) return VFS_ERR_INVAL;
    if (host_from_dev(pdev)) return VFS_OK;
...
static int i2s_client_register_impl(struct device* pdev, const void* cfg, void** out)
{
    ...
    if (!pdev || !cfg || !out) return VFS_ERR_INVAL;
    if (bus_controller_of(pdev, &ctlr) != VFS_OK) return VFS_ERR_NODEV;
    host = ctlr->hw_ctx;
    if (!host) return VFS_ERR_IO;
    id = (int)board_dev_find(device_get_name(pdev));
    if (id < 0 || id >= DEV_ID_COUNT) return VFS_ERR_INVAL;
    client = &s_clients[id];
    if (client->dev) {
        if (client->dev != pdev) return VFS_ERR_BUSY;
        *out = client;
        return VFS_OK;
    }
 ...
 * @return 成功返回 VFS_OK, 失败返回负数错误码
 */
int i2s_bus_host_init(struct device* dev, const struct hal_i2s_bus_config* cfg)
{ return i2s_host_init_impl(dev, cfg); }
 ...
 * @return 成功返回 VFS_OK, BUSY 返回 VFS_ERR_BUSY, 失败返回负数错误码
 */
int i2s_bus_host_deinit(struct device* dev)
{ return i2s_host_deinit_impl(dev); }
 ...
 * @return master 返回 bus_role 值, 失败返回 -1
 */
int i2s_bus_host_role(struct device* dev)
{ return i2s_host_role_impl(dev); }
 ...
 * @return 成功返回 VFS_OK, 失败返回负数错误码
 */
int i2s_bus_client_register(struct device* dev, const struct hal_i2s_device_config* cfg, struct i2s_bus_client** out)
{ return i2s_client_register_impl(dev, cfg, (void**)out); }
...
{ i2s_client_unregister_impl(dev); }
 ...
 * @return 成功返回 VFS_OK, 失败返回负数错误码
 */
int i2s_bus_open(struct device* dev)
{
    struct i2s_bus_client* client = client_from_dev(dev);
    ...
    {
        interrupt_virtual_register(VIRQ(i2s, host->hw_idx),
                                   hal_virtual_i2s_irq_callback,
                                   &g_i2s_bottom_half_work,
                                   &client->hal_i2s_dev);
 ...
 * @return 成功返回 VFS_OK, 失败返回负数错误码
 */
int i2s_bus_close(struct device* dev)
{
    struct i2s_bus_client* client = client_from_dev(dev);
    if (!client) return VFS_ERR_NODEV;
    if (!client->hw_open) return VFS_OK;
 ...
 * @return 成功返回 VFS_OK, 失败返回负数错误码
 */
int i2s_bus_transfer(struct device* dev, const uint16_t* tx, uint16_t* rx, size_t samples, uint32_t timeout_ms, uint32_t xfer_mode)
{
    struct i2s_bus_client* client = client_from_dev(dev);
    if (!client || !client->hw_open) return VFS_ERR_NODEV;
 ...
 * @return 成功返回 VFS_OK, 失败返回负数错误码
 */
int i2s_bus_transfer_async(struct device* dev, const uint16_t* tx, uint16_t* rx, size_t samples, void (*cb)(struct device*, const void*, void*), void* userdata)
{
    struct i2s_bus_client* client = client_from_dev(dev);
    COMPAT_IGNORE_RESULT(cb); COMPAT_IGNORE_RESULT(userdata);
    if (!client || !client->hw_open) return VFS_ERR_NODEV;
 ...
 * @return 成功返回 VFS_OK, 失败返回负数错误码
 */
int i2s_bus_transfer_poll(struct device* dev, uint32_t timeout_ms)
{
    struct i2s_bus_client* client = client_from_dev(dev);
 ...
 * @return 成功返回 VFS_OK, 失败返回负数错误码
 */
int i2s_bus_set_dma_irq_mode(struct device* dev, uint32_t irq_mode)
{
    struct i2s_bus_client* client = client_from_dev(dev);
    if (!client || !client->hw_open)
        return VFS_ERR_NODEV;
 ...
 * @return 成功返回 VFS_OK, 失败返回负数错误码
 */
int i2s_bus_get_dma_irq_mode(struct device* dev, uint32_t* irq_mode)
{
    struct i2s_bus_client* client = client_from_dev(dev);
    if (!client || !client->hw_open)
        return VFS_ERR_NODEV;
 ...
 * @return 成功返回 VFS_OK, 失败返回负数错误码
 */
int i2s_bus_dma_circ_start(struct device* dev, int tx_enable, int rx_enable)
{
    struct i2s_bus_client* client = client_from_dev(dev);
    if (!client || !client->hw_open)
        return VFS_ERR_NODEV;
 ...
 * @return 成功返回 VFS_OK, 失败返回负数错误码
 */
int i2s_bus_dma_circ_stop(struct device* dev)
{
    struct i2s_bus_client* client = client_from_dev(dev);
    if (!client || !client->hw_open)
        return VFS_ERR_NODEV;
 ...
 * @return 成功返回 VFS_OK, 失败返回负数错误码
 */
int i2s_bus_dma_circ_write(struct device* dev, const uint16_t* data, uint32_t samples)
{
    struct i2s_bus_client* client = client_from_dev(dev);
    if (!client || !client->hw_open)
        return VFS_ERR_NODEV;
 ...
 * @return 成功返回 VFS_OK, 失败返回负数错误码
 */
int i2s_bus_dma_circ_read(struct device* dev, uint16_t* data, uint32_t samples)
{
    struct i2s_bus_client* client = client_from_dev(dev);
    if (!client || !client->hw_open)
        return VFS_ERR_NODEV;
