 ...
 */
int  usb_bus_host_init(struct device* dev, const struct hal_usb_bus_config* cfg) COMPAT_WARN_UNUSED_RESULT;
/**
 * @brief USB host 反初始化 (仍有 client 引用时返回 BUSY)
 * @param dev controller device (host)
 * @return VFS_OK / VFS_ERR_BUSY / VFS_ERR_*
 */
int  usb_bus_host_deinit(struct device* dev) COMPAT_WARN_UNUSED_RESULT;
 ...
 * @param cls CDC / ECM / HID
 * @param out 输出 client 句柄
 * @return VFS_OK 或 VFS_ERR_*
 */
int  usb_bus_client_register(struct device* dev, enum usb_client_class cls,
                             struct usb_bus_client** out) COMPAT_WARN_UNUSED_RESULT;
/**
 * @brief 注销 USB class client, ref_count -1
 * @param dev client device
 */
void usb_bus_client_unregister(struct device* dev);
 ...
 * @return VFS_OK 或 VFS_ERR_*
 */
int  usb_bus_open(struct device* dev) COMPAT_WARN_UNUSED_RESULT;
/**
 * @brief 关闭 client (幂等)
 * @param dev client device
 * @return VFS_OK 或 VFS_ERR_*
 */
int  usb_bus_close(struct device* dev) COMPAT_WARN_UNUSED_RESULT;

/**
 * @brief 解析传输路径 (AUTO/POLL/DMA)
 * @param client_or_host client 或 host device
 * @param xfer_mode HAL_USB_XFER_*
 ...
 * @param buf 数据
 * @param len 长度
 * @param timeout_ms 超时毫秒; 0 表示非阻塞尝试
 * @param xfer_mode HAL_USB_XFER_*
