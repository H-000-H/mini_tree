...
各节点项目已通过 `CMakePresets.json` 绑定正确的工具链文件，主环境为 Linux，路径由 `find_program` 自动探测（Linux / Docker / Windows 三端均可）：
...
mini_tree 自身已根据 `.config` 中 `CONFIG_HAL_GPIO_<STM32|CH32|ESP32>` 等选项自动选择对应平台目录下的 HAL 实现文件（如 `hal/spi/hal_spi_stm32.c`、`hal/uart/hal_uart_ch32.c`），无需用户工程通过 `HAL_SRCS` 变量手动收集。
...
- 若引入新芯片厂商，需在 `hal/<periph>/` 下补充 `hal_<periph>_<chip>.c` 实现，并在 mini_tree 的 `CMakeLists.txt` 中按 `CONFIG_HAL_GPIO_<VENDOR>` 分支注册
...
> 框架 HAL 头文件保持厂商中性（仅含 `uintptr_t`/`int`/`void*` 与结构体定义），厂商头文件由 `hal_<periph>_<chip>.c` 内部 `#include` 并内部 cast。
...
| | `SYSTEM_C` | 纯 C system |
| **HAL Vendor** | `CONFIG_HAL_GPIO_STM32` | STM32 平台 HAL 实现 |
| | `CONFIG_HAL_GPIO_CH32` | WCH 平台 HAL 实现 |
| | `CONFIG_HAL_GPIO_ESP32` | ESP32 平台 HAL 实现 |
...
```
my_project/
├── CMakeLists.txt              # 项目根：工具链、BOARD_DTS、应用层
├── CMakePresets.json           # Debug/Release 预设
├── mini_tree/                  # 框架子目录（含 board/dts/、board/dtsi/、dt-bindings/）
│   ├── CMakeLists.txt          # 框架构建脚本（按 CONFIG_HAL_GPIO_<VENDOR> 选择 HAL 实现）
│   ├── Kconfig
│   ├── hal/                    # 通用 HAL 头 + 平台 HAL .c 实现
│   │   ├── spi/hal_spi.h
│   │   ├── spi/hal_spi_stm32.c
│   │   ├── uart/hal_uart.h
│   │   └── uart/hal_uart_stm32.c
│   └── ...
...
# 引入 mini_tree（框架内部按 .config 自动选择平台 HAL 实现）
add_subdirectory(mini_tree)
...
具体芯片的 HAL 实现放在 `mini_tree/hal/<periph>/` 目录下，文件名遵循 `hal_<periph>_<chip>.c` 约定。框架的 `CMakeLists.txt` 根据 `CONFIG_HAL_GPIO_<VENDOR>` 选项自动选择对应平台目录下的实现文件，**不再使用 `HAL_SRCS` 变量**统一收集。框架层只依赖 `hal/<periph>/hal_<periph>.h` 声明的统一 API，业务层和 bus 层无需感知具体芯片：

```c
/* hal/spi/hal_spi_stm32.c — STM32 平台 SPI HAL 实现 */
#include "hal_spi.h"              /* 框架头: 厂商中性, 仅含 uintptr_t/int/void* */
#include "VFS.h"
#include "osal.h"

/* 厂商 SDK 头文件仅在 .c 内部 include */
#include "stm32f4xx.h"
...
{
    SPI_TypeDef* spi = (SPI_TypeDef*)cfg->spi;   /* uintptr_t → vendor ptr */
             ...
             size_t len, uint32_t timeout_ms)
{
    ...
    SPI_TypeDef* spi = (SPI_TypeDef*)host->spi;   /* 直接解引用缓存值 */
    /* CS 变更检测: cs_port + cs_pin + mode + clock_speed_hz 任一变化触发重配 */
    /* ... 调用 LL 库执行同步传输 ... */
    return VFS_OK;
}
```
...
```c
/* hal/uart/hal_uart_stm32.c — STM32 平台 UART HAL 实现 */
#include "hal_uart.h"
#include "stm32f4xx.h"
...
{
    USART_TypeDef* uart = (USART_TypeDef*)dev->uart;   /* 直接解引用缓存值 */
...
> 命名规范：所有 HAL 函数使用 `hal_<periph>_*` 前缀（如 `hal_spi_bus_host_init`、`hal_uart_dev_init`、`hal_gpio_fast_set_level`），不再使用旧的 `spi_hal_*` / `uart_hal_*` 命名。
