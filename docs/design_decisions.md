...
| **Keil Studio**（基于 VS Code / VS Code-based） | **理论支持（待 Windows 验证，接入自理）/ Theoretically supported (pending Windows validation, DIY)** | 与 µVision 本质不同：VS Code 内核 + CMake 一等公民（CMSIS-Toolbox / cbuild），可装 clangd 扩展，保留官方调试与云编译；但作者在 Linux 实测遇本机问题、尚未在 Windows 验证——接入请自行完成。 / Fundamentally different from µVision: VS Code core + first-class CMake (CMSIS-Toolbox / cbuild), clangd extension available, official debug & cloud build kept; however the author hit machine-specific issues on Linux and has not validated on Windows — integrate at your own pace. |
| **传统 Keil 5**：**Keil µVision** | **不支持 / Not supported** | 几乎无法干净集成；跳转/C++/AI 弱。降级时最简单是 **自写/自改 Python 生成 `.uvprojx`**（远古有过类似思路，**现已不维护**）。详见 [keil_integration.md](keil_integration.md)。 / Nearly impossible to integrate cleanly; weak navigation/C++/AI. The simplest fallback is a **self-written/modified Python script generating `.uvprojx`** (a legacy idea, **no longer maintained**). See [keil_integration.md](keil_integration.md). |
| **ARMCC v5** | **不支持 / Unsupported** | 过旧，缺现代 C/GNU 扩展。 / Too old; lacks modern C/GNU extensions. |

作者偏好：**用现代编辑器写代码，用 CMake 出固件**；经典 Keil µVision（**传统 Keil 5**）**不支持**、不作受支持路径，若必须交付 µVision 工程，自备 Python 生成 `.uvprojx`（见 [keil_integration.md](keil_integration.md)），作者不维护该生成器；**Keil Studio 单独列为理论支持项**（VS Code 内核 + CMake 一等公民；作者在 Linux 实测遇本机问题、待 Windows 验证，接入自理，见 [keil_integration.md](keil_integration.md) §2.1）。
Author's preference: **write code in a modern editor, build firmware with CMake**; classic Keil µVision (**traditional Keil 5**) is **NOT supported** and is not a supported path — if a µVision project must be delivered, bring your own Python generator for `.uvprojx` (see [keil_integration.md](keil_integration.md)); the author does not maintain that generator. **Keil Studio is listed separately as a theoretically supported entry** (VS Code core + first-class CMake; the author hit machine-specific issues on Linux and it awaits Windows validation — DIY, see [keil_integration.md](keil_integration.md) §2.1).
