...
| **Zephyr** | **不推荐** | DTS→宏/代码生成路径怪，比「纯粹 Linux 设备树用法」更绕，调试成本高；本仓 **不做** Zephyr OSAL 后端，也不对标其 dts 工具链。 |
...
| **Keil µVision** | **不推荐；已不支持** | 几乎无法干净集成；跳转/C++/AI 弱。降级时最简单是 **自写/自改 Python 生成 `.uvprojx`**（远古有过类似思路，**现已不维护**）。详见 [keil_integration.md](keil_integration.md)。 |
| **ARMCC v5** | **不支持** | 过旧，缺现代 C/GNU 扩展。 |
...
学 Linux / ESP 的 **分层与 VFS 心智**，用 FreeRTOS（或 RT-Thread 组件）做 **调度**，用本仓 dtc-lite 做 **编译期、可裁剪的板级描述**——刻意避开 Zephyr 那套「为嵌入式再发明一套更重的 dts 体验」。开发侧则避开 Keil 当主战场，留在 **Cursor / VS Code / CLion / Qoder** 一类现代工具上。
