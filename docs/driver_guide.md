...
- **`board/dtsi/<chip>.dtsi`** — SoC 级定义（芯片厂商提供），声明所有外设 IP 块，默认 `status = "disabled"`
- **`board/dts/<board>.dts`** — 板级定义，`#include` SoC 模板，用 `&label` 覆写使能所需外设

```dts
/* board/dtsi/template.dtsi — SoC 模板 */
/ {
    soc {
        #address-cells = <1>;
        #size-cells = <1>;
...
移植到新芯片时只需创建新的 `dtsi/<chip>.dtsi`，board.dts 改用对应的 `#include`。

> 兼容字符串统一无平台前缀：`spi-master`、`spi-slave`、`uart`、`gpio`、`*-platform-cap`（如 `spi-platform-cap`、`uart-platform-cap`），不再有 `stm32,`/`ch32,`/`esp32,` 前缀。
...
| 属性 | 作用 | 取值示例 |
|------|------|---------|
| `compatible` | 驱动匹配字 | `"uart"` / `"spi-master"` / `"gpio"` / `"*-platform-cap"` |
...
DRIVER_REGISTER(my_uart, "uart", my_uart_probe, my_uart_remove);
...
int my_driver_probe(struct device* dev)
{
    ...
    /* 第 3 步: 通过 bus 层 API 初始化硬件 (不直接调 HAL) */
    struct hal_spi_bus_config host_cfg = { .spi = (uintptr_t)SPI1_BASE, /* ... */ };
    if (spi_bus_host_init(dev, &host_cfg) != VFS_OK) {
        ret = -1;
        goto cleanup_rx;
    }
...
> 驱动 Probe 应通过 bus 层 API（如 `spi_bus_host_init`、`spi_bus_client_register`）操作硬件，不直接调用 `hal_spi_bus_host_init` 等 HAL 函数——分层隔离由 `#pragma GCC poison` 强制。
