...
> 相对本仓根目录「通用 CMake」（`add_subdirectory` + `add_library(mini_tree STATIC)`），ESP32 走 **IDF 组件** 路径。参考实现：`platform/Espressif/esp32s3/`（`components/mini_tree` + 可选 `components/driver_ws2812`）。
...
本仓库（shelf：`/home/ning/project/shelf/mini_tree` 或平台仓内副本）与 `platform/Espressif/esp32s3/components/mini_tree` 应对齐。
