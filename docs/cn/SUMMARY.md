...
关键术语保留原文：`Device Tree (DTS/DTSI)`、`DRIVER_REGISTER`、`dtc-lite`、`OSAL (NULL/FREERTOS)`、`VFS`、`BUS`、`HAL`、`EventBus`、`VIRQ`。
...
| `ecosystem.md` | 生态：`lib/` 仅 vendor **ETL**；其余第三方库走 ESP-IDF Component Manager / registry | **P0** | [cn](ecosystem.md) |
...
| `osal_switching.md` | OSAL 后端切换（NULL/FREERTOS；优先级语义随后端变化） | P1 | [cn](osal_switching.md) |
...
| `tools_guide.md` | `dtc-lite` / scrubber stub 用法；`dtc-lite` 支持 `-I`/`-D` | **P0**（工具链） | [cn](tools_guide.md) |
...
- **P2（深入）**：`usage.md` · `faq.md` · `amp.md` · `can_hook.md` · `memory_footprint.md` · `api_compatibility.md` · `references.md` · `problem_summary.md` · `roadmap.md` · `todolist.md` · `board_linux_vs_device_model.md`
...
| OSAL 后端 | `CONFIG_OSAL_FREERTOS`（默认，IDF 内置）/ `CONFIG_OSAL_NULL`（裸机后备） |
| 平台 | ESP32（本分支纯 ESP） |
