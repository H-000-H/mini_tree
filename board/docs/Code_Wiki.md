...
- `spi_bus.c`: 注册 `DRIVER_REGISTER(spi_bus, "esp32,spi", ...)`。职责：解析 host-id / mosi / miso / sclk / dma-tx-cfg / dma-rx-cfg → `hal_spi_bus_host_init` → 绑定 `bus_controller_bind` → 自动 enumerate 挂在本 host 下的 child (cascade)。
