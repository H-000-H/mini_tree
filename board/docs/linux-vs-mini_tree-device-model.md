...
| L5 子系统  | MTD / input / misc               | 板级功能驱动 (如 `w25q64_spi_drv`)    |
| L6 资源   | `devm_*`                         | HAL + DTS props                 |
| L7 DT   | `device_node` + DTB              | `s_nodes`（dtc-lite）             |
| L8 HW   | SoC 驱动                           | 厂商 HAL/LL 库 (各平台提供)        |
...
| `spi_device`            | 子设备 `s_devices`（如 SPI Flash）          |
...
`w25q64_spi_drv.c` = 功能驱动；`vfs-spi.c` = 总线客户端 + fops。
...
各平台 HAL（STM32 LL / ESP-IDF / CH32 库等）封装上述细节。
...
| 5   | `bus/spi/spi_bus.c`                    | spi_bus_type + controller |
| 6   | `vfs/spi/vfs-spi.c`                    | spi_device + fops     |
...
*文档版本：v2.0 — Linux 详解版 + mini_tree 中间件对照。*