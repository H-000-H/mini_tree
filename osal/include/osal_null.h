...
#if defined(__cplusplus) && defined(CONFIG_OSAL_NULL_TASK_CPP) && !defined(CONFIG_XTASK_PREEMPT)
 ...
 *
 * 调度器后端互斥: 本声明仅在 CONFIG_XTASK_PREEMPT 未定义 (协调式) 时可见;
 * 开启抢占式后, 协调式重载 (osal_task.cpp) 整段关闭, 改由抢占式专用重载提供
 * (签名不同 — 抢占式有优先级概念, period 参数语义变化). */
