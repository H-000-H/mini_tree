...
| 中间件层 → 硬件 | 不直接操作寄存器，通过 `hal_if/` 接口委派给用户实现 |

### 1.4 构建系统的架构独立性

传统单片机开发重度依赖 IDE 工程文件（如 Keil 的 `.uvprojx` 或 IAR 的 `.uvoptx`）管理代码依赖。本架构采用不同的构建策略：

- **传统 IDE 的局限**：源文件管理、编译宏定义被绑定在不可读的 XML/私有格式文件中，导致多平台移植困难，且难以接入现代 CI/CD 自动化流程。
- **本架构的解法 (Kbuild 思想)**：构建权完全上收至 `CMakeLists.txt` 和独立 `Makefile`，配合 `Kconfig` 菜单配置。代码只负责实现逻辑，由外部构建系统决定物理裁剪。即使需要交付 Keil 工程，底层的源文件挂载也应通过自动化脚本生成，阻断 IDE 界面对架构隔离层的干扰。
...
> **向下兼容确认**: 实际验证最低 C11、C++17 可正常编译，更低版本（C89/C99、C++98/11）未做验证。建议用户工程至少使用 C11 或 C++17。
