...
> 相对本仓根目录「通用 CMake」（`add_subdirectory` + `add_library(mini_tree STATIC)`），ESP32 走 **IDF 组件** 路径。参考实现：`platform/Espressif/esp32s3/`（`components/mini_tree` + `components/board_port.cmake` + `components/hal_esp32s3` + 可选 `components/driver_ws2812`）。
...
| :--- | :--- | :--- |
| 接入 | `add_subdirectory(mini_tree)` | `components/mini_tree` 自动扫描；`ESP_PLATFORM` 时走 `cmake/esp_idf.cmake` 并 `return()` |
...
本仓库（shelf）与 ESP 板工程的 `components/mini_tree` 应对齐。

建议：

1. 在 **ESP 板工程**完成产品驱动 / DTS / CMake 验收（`idf.py build`）。
2. **内容同步到 shelf**（`rsync` 添加/覆盖，**不要** `--delete` 掉 shelf 独有如 `board/docs`；**排除** `.git` / `.config`）。
3. 反向：shelf 改通用层后再拷回板工程时，保留板级 `hal_esp32s3`、根工程 `CMakeLists.txt`、`driver_ws2812`、`board_port.cmake`。
...
- [ ] `sdkconfig.defaults` 含 `CONFIG_PLATFORM_ESP32=y`（平台声明必选）
