 ...
 */
pre_execution(160) static void sx1278_pool_boot_init(void)
{
 ...
 */
static int sx1278_spi_xfer(struct sx1278_device* d, const uint8_t* tx, uint8_t* rx, size_t len,
                           uint32_t to)
{
...
static int sx1278_wr_reg(struct sx1278_device* d, uint8_t reg, uint8_t val, uint32_t to)
{
    ...
    return sx1278_spi_xfer(d, tx, NULL, 2, to);
}
/**
...
static int sx1278_cmd_freq(struct sx1278_device* d, void* arg, size_t len, uint32_t to)
{
    ...
    if (!arg || len != sizeof(uint32_t))
        return VFS_ERR_INVAL;
    hz = *(uint32_t*)arg;
    f = ((uint64_t)hz << 19) / 32000000ULL;
    frf[0] = (uint8_t)(f >> 16);
    frf[1] = (uint8_t)(f >> 8);
    frf[2] = (uint8_t)f;
    {
        uint8_t tx[4] = {0x06 | 0x80U, frf[0], frf[1], frf[2]};
        if (sx1278_spi_xfer(d, tx, NULL, 4, to) != VFS_OK)
            return VFS_ERR_IO;
    }
    return VFS_OK;
}
...
static int sx1278_cmd_send(struct sx1278_device* d, void* arg, size_t len, uint32_t to)
{
    ...
    if (!pl || len != sizeof(*pl) || !pl->data || !pl->len)
        return VFS_ERR_INVAL;
    if (sx1278_wr_reg(d, 0x01, 0x83, to) != VFS_OK)
        return VFS_ERR_IO;
    {
        uint8_t tx[257];
        size_t n = pl->len;
        if (n > 255U)
            n = 255U;
        tx[0] = 0x80;
        COMPAT_IGNORE_RESULT(COMPAT_MEM_COPY(&tx[1], pl->data, n));
        if (sx1278_spi_xfer(d, tx, NULL, n + 1U, to) != VFS_OK)
            return VFS_ERR_IO;
    }
    return VFS_OK;
}
