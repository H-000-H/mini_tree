...
{
    MINI_IGNORE_RESULT(osal_pool_init(&s_ads1115_pool_ctrl, s_ads1115_used, ADS1115_POOL_COUNT));
}
...
err:
    pdev->ops = NULL;
    MINI_MEM_SET(dev, 0, sizeof(*dev));
    MINI_IGNORE_RESULT(osal_pool_release(&s_ads1115_pool_ctrl, pool_idx));
    return ret;
}
...
static int ads1115_remove(struct device* pdev)
{
    ...
    ads1115_hw_destroy(dev);
    MINI_MEM_SET(dev, 0, sizeof(*dev));
    MINI_IGNORE_RESULT(osal_pool_release(&s_ads1115_pool_ctrl, idx));
    dev_lc_remove_finish(lc);
    return MINI_OK;
}
