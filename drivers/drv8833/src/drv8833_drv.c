...
static int drv8833_hw_create(struct drv8833_device* d)
{
    ...
    { struct device* g[4]={d->ain1_dev, d->ain2_dev, d->bin1_dev, d->bin2_dev};
      struct vfs_gpio_arg* a[4]={&d->ain1, &d->ain2, &d->bin1, &d->bin2}; int i, r;
      for(i=0;i<4;i++){ r=device_open(g[i], NULL); if(r!=VFS_OK)return r;
...
static void drv8833_hw_destroy(struct drv8833_device* d)
{
    if (!d || !d->hw_ready)
        return;
    if(d->ain1_dev) COMPAT_IGNORE_RESULT(device_close(d->ain1_dev));
    if(d->ain2_dev) COMPAT_IGNORE_RESULT(device_close(d->ain2_dev));
    if(d->bin1_dev) COMPAT_IGNORE_RESULT(device_close(d->bin1_dev));
    if(d->bin2_dev) COMPAT_IGNORE_RESULT(device_close(d->bin2_dev));
    d->hw_ready=0;
...
static int drv8833_cmd_motor(struct drv8833_device* d, void* arg, size_t len, uint32_t ms)
{
    ...
    if(!d->hw_ready||!m||len!=sizeof(*m)) return VFS_ERR_INVAL;
    if(m->motor==0){ drv8833_apply(&d->ain1, m->fwd); drv8833_apply(&d->ain2, !m->fwd); }
    else { drv8833_apply(&d->bin1, m->fwd); drv8833_apply(&d->bin2, !m->fwd); }
    return VFS_OK;
}
...
static int drv8833_probe(struct device* dev)
{
    ...
    d->ain1_dev=device_get_phandle_dev(dev, "ain1-gpio");
    d->ain2_dev=device_get_phandle_dev(dev, "ain2-gpio");
    d->bin1_dev=device_get_phandle_dev(dev, "bin1-gpio");
    d->bin2_dev=device_get_phandle_dev(dev, "bin2-gpio");
