...
{
    MINI_IGNORE_RESULT(osal_pool_init(&s_aht20_pool_ctrl, s_aht20_used, AHT20_POOL_COUNT));
}
...
err:
    pdev->ops = NULL;
    MINI_MEM_SET(dev, 0, sizeof(*dev));
    MINI_IGNORE_RESULT(osal_pool_release(&s_aht20_pool_ctrl, pool_idx));
    return ret;
}
...
static int aht20_remove(struct device* pdev)
{
    ...
    aht20_hw_destroy(dev);
    MINI_MEM_SET(dev, 0, sizeof(*dev));
    MINI_IGNORE_RESULT(osal_pool_release(&s_aht20_pool_ctrl, idx));
    dev_lc_remove_finish(lc);
    return MINI_OK;
}
