...
{
    MINI_IGNORE_RESULT(osal_pool_init(&s_sht30_pool_ctrl, s_sht30_used, SHT30_POOL_COUNT));
}
...
err:
    pdev->ops = NULL;
    MINI_MEM_SET(dev, 0, sizeof(*dev));
    MINI_IGNORE_RESULT(osal_pool_release(&s_sht30_pool_ctrl, pool_idx));
    return ret;
}
...
static int sht30_remove(struct device* pdev)
{
    ...
    sht30_hw_destroy(dev);
    MINI_MEM_SET(dev, 0, sizeof(*dev));
    MINI_IGNORE_RESULT(osal_pool_release(&s_sht30_pool_ctrl, idx));
    dev_lc_remove_finish(lc);
    return MINI_OK;
}
