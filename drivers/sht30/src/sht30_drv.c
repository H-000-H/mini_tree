/* SPDX-License-Identifier: Apache-2.0 */
#include "sht30_drv.h"
#include "vfs-i2c.h"
#include "device.h"
#include "driver.h"
#include "dev_lifecycle.h"
#include "status.h"
#include "dt_config_gen.h"
#include "compiler_compat.h"
#include "osal.h"
#include "system_log.h"
#include <stddef.h>
#include <stdint.h>
#include "compiler_compat_poison.h"

#ifndef DTC_GEN_COUNT_SENSIRION_SHT30
#define DTC_GEN_COUNT_SENSIRION_SHT30  1
#endif
#define SHT30_POOL_COUNT  DTC_GEN_COUNT_SENSIRION_SHT30

struct sht30_device
{
    struct file_operations ops;
    struct device*         i2c_dev;

    int                    hw_ready;
};

static struct sht30_device s_sht30_pool[SHT30_POOL_COUNT] COMPAT_ALIGNED(4);
static uint8_t             s_sht30_used[SHT30_POOL_COUNT] COMPAT_ALIGNED(4);
static osal_pool_t         s_sht30_pool_ctrl COMPAT_ALIGNED(4);
static const char* const kTag = "sht30";

pre_execution(160)
static void sht30_pool_boot_init(void)
{
    COMPAT_IGNORE_RESULT(osal_pool_init(&s_sht30_pool_ctrl, s_sht30_used, SHT30_POOL_COUNT));
}

static struct sht30_device* sht30_get_drvdata(struct device* dev)
{
    return (struct sht30_device*)device_get_priv(dev);
}


static int sht30_i2c_wr(struct sht30_device* d, const uint8_t* tx, size_t len, uint32_t to)
{
    if (!d || !d->i2c_dev || !tx || len == 0U)
        return VFS_ERR_INVAL;
    return device_write(d->i2c_dev, tx, len, to);
}

static int sht30_i2c_rd(struct sht30_device* d, uint8_t* rx, size_t len, uint32_t to)
{
    if (!d || !d->i2c_dev || !rx || len == 0U)
        return VFS_ERR_INVAL;
    return device_read(d->i2c_dev, rx, len, to);
}


static int sht30_hw_create(struct sht30_device* d)
{
    if (!d)
        return VFS_ERR_INVAL;
    if (d->hw_ready)
        return VFS_OK;
    { int r = device_open(d->i2c_dev, NULL); if (r != VFS_OK) return r; }
    d->hw_ready = 1; return VFS_OK;

}

static void sht30_hw_destroy(struct sht30_device* d)
{
    if (!d || !d->hw_ready)
        return;
    if (d->i2c_dev) COMPAT_IGNORE_RESULT(device_close(d->i2c_dev));
    d->hw_ready = 0;

}

static int sht30_open(struct device* dev, void* arg)
{
    struct sht30_device* d;
    struct dev_lifecycle* lc;
    int first, ret;
    COMPAT_IGNORE_RESULT(arg);
    if (!dev || !dev->ops)
        return VFS_ERR_INVAL;
    d = sht30_get_drvdata(dev);
    if (IS_ERR(d))
        return PTR_ERR(d);
    lc = device_lc(dev);
    if (IS_ERR(lc))
        return PTR_ERR(lc);
    first = dev_lc_open_begin(lc);
    if (first < 0)
        return first;
    ret = VFS_OK;
    if (first == 1)
    {
        ret = sht30_hw_create(d);
        if (ret != VFS_OK)
        {
            dev_lc_open_abort(lc);
            return ret;
        }
    }
    dev_lc_open_end(lc);
    return VFS_OK;
}

static int sht30_close(struct device* dev)
{
    struct sht30_device* d;
    struct dev_lifecycle* lc;
    int last;
    if (!dev || !dev->ops)
        return VFS_ERR_INVAL;
    d = sht30_get_drvdata(dev);
    if (IS_ERR(d))
        return PTR_ERR(d);
    lc = device_lc(dev);
    if (IS_ERR(lc))
        return PTR_ERR(lc);
    last = dev_lc_close_begin(lc);
    if (last < 0)
        return last;
    if (last)
        sht30_hw_destroy(d);
    dev_lc_close_end(lc);
    return VFS_OK;
}

typedef int (*sht30_ioctl_fn_t)(struct sht30_device* d, void* arg, size_t arg_len, uint32_t ms);
struct sht30_ioctl_map { sht30_ioctl_fn_t handler; };


static int sht30_cmd_read(struct sht30_device* d, void* arg, size_t len, uint32_t to)
{
    const uint8_t cmd[2]={0x24, 0x00}; uint8_t raw[6]; struct sht30_sample* o=(struct sht30_sample*)arg; int r;
    if(!d->hw_ready||!o||len!=sizeof(*o)) return VFS_ERR_INVAL;
    r=sht30_i2c_wr(d, cmd, 2, to); if(r!=VFS_OK) return r; osal_delay_ms(20);
    r=sht30_i2c_rd(d, raw, 6, to); if(r!=VFS_OK) return r;
    { uint16_t t=(uint16_t)((raw[0]<<8)|raw[1]), h=(uint16_t)((raw[3]<<8)|raw[4]);
      o->temp_c_x100=(int16_t)((((int32_t)t*17500)/65535)-4500);
      o->rh_x100=(uint16_t)(((uint32_t)h*10000U)/65535U); }
    return VFS_OK;
}
static const struct sht30_ioctl_map s_sht30_map[SHT30_CMD_COUNT] = {
    [SHT30_CMD_READ_TEMP_RH - SHT30_CMD_BASE - 1] = { sht30_cmd_read },
};


static int sht30_ioctl(struct device* dev, int cmd, void* arg, size_t arg_len, uint32_t ms)
{
    struct sht30_device* d;
    struct dev_lifecycle* lc;
    int32_t off;
    int ret;
    if (!dev || !dev->ops)
        return VFS_ERR_INVAL;
    d = sht30_get_drvdata(dev);
    if (IS_ERR(d))
        return PTR_ERR(d);
    lc = device_lc(dev);
    if (IS_ERR(lc))
        return PTR_ERR(lc);
    ret = dev_lc_io_begin(lc);
    if (ret != VFS_OK)
        return ret;
    off = (int32_t)cmd - (int32_t)SHT30_CMD_BASE;
    if (off < 1 || off > SHT30_CMD_COUNT || !s_sht30_map[off - 1].handler)
        ret = VFS_ERR_INVAL;
    else
        ret = s_sht30_map[off - 1].handler(d, arg, arg_len, ms);
    dev_lc_io_end(lc);
    return ret;
}

static const struct file_operations sht30_fops =
{
    .open  = sht30_open,
    .close = sht30_close,
    .ioctl = sht30_ioctl,
};

static int sht30_probe(struct device* dev)
{
    struct sht30_device* d;
    int pool_idx, ret;
    if (!dev)
        return VFS_ERR_INVAL;
    pool_idx = osal_pool_claim(&s_sht30_pool_ctrl);
    if (pool_idx < 0)
        return VFS_ERR_NOMEM;
    d = &s_sht30_pool[pool_idx];
    COMPAT_MEM_SET(d, 0, sizeof(*d));
    d->i2c_dev = device_get_parent(dev);
    if (!d->i2c_dev) { ret = VFS_ERR_NODEV; goto err; }

    if (device_set_priv(dev, d) != VFS_OK)
    {
        ret = VFS_ERR_IO;
        goto err;
    }
    d->ops = sht30_fops;
    dev->ops = &d->ops;
    SYS_LOGI(kTag, "probe OK pool=%d", pool_idx);
    return VFS_OK;
err:
    dev->ops = NULL;
    COMPAT_MEM_SET(d, 0, sizeof(*d));
    COMPAT_IGNORE_RESULT(osal_pool_release(&s_sht30_pool_ctrl, pool_idx));
    return ret;
}

static int sht30_remove(struct device* dev)
{
    struct sht30_device* d;
    struct dev_lifecycle* lc;
    int idx;
    if (!dev)
        return VFS_ERR_INVAL;
    d = sht30_get_drvdata(dev);
    if (IS_ERR(d))
        return PTR_ERR(d);
    lc = device_lc(dev);
    if (IS_ERR(lc))
        return PTR_ERR(lc);
    idx = (int)(d - s_sht30_pool);
    dev_lc_remove_start(lc);
    device_ops_unregister(dev);
    if (dev_lc_remove_drain(lc, OSAL_WAIT_FOREVER) != VFS_OK)
    {
        dev_lc_remove_finish(lc);
        return VFS_ERR_IO;
    }
    sht30_hw_destroy(d);
    COMPAT_MEM_SET(d, 0, sizeof(*d));
    COMPAT_IGNORE_RESULT(osal_pool_release(&s_sht30_pool_ctrl, idx));
    dev_lc_remove_finish(lc);
    return VFS_OK;
}

DRIVER_REGISTER(sht30, "sensirion,sht30", sht30_probe, sht30_remove)
