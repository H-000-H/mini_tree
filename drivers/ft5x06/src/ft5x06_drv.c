 ...
 */
pre_execution(PRE_EXEC_PRIO_DRIVER_POOL) static void ft5x06_pool_boot_init(void) { COMPAT_IGNORE_RESULT(osal_pool_init(&s_ft5x06_pool_ctrl, s_ft5x06_used, FT5X06_POOL_COUNT)); }
...
static int ft5x06_cmd_touch(struct ft5x06_device* dev, void* arg, size_t len, uint32_t timeout_ms)
{
    ...
    if (!dev->hw_ready || !t || len != sizeof(*t))
        return VFS_ERR_INVAL;
    if (ft5x06_i2c_wr(dev, &reg, 1, timeout_ms) != VFS_OK)
        return VFS_ERR_IO;
    if (ft5x06_i2c_rd(dev, raw, 6, timeout_ms) != VFS_OK)
        return VFS_ERR_IO;
