...
static osal_pool_t s_a7670_pool_ctrl MINI_ALIGNED(4);
...
{
    MINI_IGNORE_RESULT(osal_pool_init(&s_a7670_pool_ctrl, s_a7670_used, A7670_POOL_COUNT));
}
...
err:
    pdev->ops = NULL;
    MINI_MEM_SET(dev, 0, sizeof(*dev));
    MINI_IGNORE_RESULT(osal_pool_release(&s_a7670_pool_ctrl, pool_idx));
    return ret;
}
...
static int a7670_remove(struct device* pdev)
{
    ...
    a7670_hw_destroy(dev);
    MINI_MEM_SET(dev, 0, sizeof(*dev));
    MINI_IGNORE_RESULT(osal_pool_release(&s_a7670_pool_ctrl, idx));
    dev_lc_remove_finish(lc);
    return MINI_OK;
}
