...
static osal_pool_t s_hc05_pool_ctrl MINI_ALIGNED(4);
...
{
    MINI_IGNORE_RESULT(osal_pool_init(&s_hc05_pool_ctrl, s_hc05_used, HC05_POOL_COUNT));
}
...
err:
    pdev->ops = NULL;
    MINI_MEM_SET(dev, 0, sizeof(*dev));
    MINI_IGNORE_RESULT(osal_pool_release(&s_hc05_pool_ctrl, pool_idx));
    return ret;
}
...
static int hc05_remove(struct device* pdev)
{
    ...
    hc05_hw_destroy(dev);
    MINI_MEM_SET(dev, 0, sizeof(*dev));
    MINI_IGNORE_RESULT(osal_pool_release(&s_hc05_pool_ctrl, idx));
    dev_lc_remove_finish(lc);
    return MINI_OK;
}
