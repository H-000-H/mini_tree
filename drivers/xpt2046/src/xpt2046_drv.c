 ...
 */
pre_execution(160) static void xpt2046_pool_boot_init(void)
{
 ...
 */
static int xpt2046_spi_xfer(struct xpt2046_device* d, const uint8_t* tx, uint8_t* rx, size_t len,
                            uint32_t to)
{
...
static int xpt2046_cmd_xy(struct xpt2046_device* d, void* arg, size_t len, uint32_t to)
{
    ...
    if (!d->hw_ready || !p || len != sizeof(*p))
        return VFS_ERR_INVAL;
    if (xpt2046_spi_xfer(d, tx, rx, 3, to) != VFS_OK)
        return VFS_ERR_IO;
    p->x = (uint16_t)(((rx[1] << 8) | rx[2]) >> 3);
    tx[0] = 0xD0;
    if (xpt2046_spi_xfer(d, tx, rx, 3, to) != VFS_OK)
        return VFS_ERR_IO;
    p->y = (uint16_t)(((rx[1] << 8) | rx[2]) >> 3);
    p->pressed = 1;
    return VFS_OK;
}
...
static int xpt2046_probe(struct device* pdev)
{
    ...
    d->irq_dev = device_get_phandle_dev(pdev, "irq-gpio");
    if (IS_ERR(d->irq_dev))
    {
        d->irq_dev = NULL;
        d->has_irq = 0;
