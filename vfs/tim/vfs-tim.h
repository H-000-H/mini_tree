 ...
 * 隔离: 本文件定义 TIM_VFS_IMPL 可调 tim_hal API; 其他文件包含本头时 tim_hal 符号被 #pragma GCC poison。
 *
 * Driver 注册:
 *   - tim_vfs: "tim"
 * TIM和I2C SPI 这些需要总线的设备不同,不需要挂载到总线,直接挂载到VFS层,通过文件操作接口进行操作。和gpio类似,TIM和GPIO都是通过文件操作接口进行操作。
 ...
 * - 其他文件: hal_tim_* 热路径函数 (pwm/counter/capture/encoder/hall/arr/prescaler) 允许内联快路径直透
