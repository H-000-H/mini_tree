...
| 文件 | 说明 |
|------|------|
| `hal_cpu.c` | CPU 周期计数器、忙等待移植模板 |
| `hal_flash.c` | Flash 读取移植模板 |
| `hal_force_stop.c` | 紧急外设停机移植模板 |
| `hal_storage.c` | 持久化配置存储移植模板 |
| `hal_wdt.c` | 看门狗移植模板 |
| `hal_platform_safety.c` | 故障 LED/蜂鸣器硬件锁定移植模板 |

各模板展示了同一套 DTS 适配模式 —— **SPI 四线整体换引脚**：
...
- 不同开发风格 (HAL/LL/SPL/寄存器) 对同一 DTS 属性的读取方式一致
