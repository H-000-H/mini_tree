...
enum
{
    VIRTUAL_IRQ_BLOCK_TABLE(BLOCK_INDEX_ENUM)
    VIRTUAL_BLOCK_COUNT
};
...
typedef enum
{
    VIRTUAL_IRQ_BLOCK_TABLE(VIRTUAL_IRQ_BLOCK_ENUM)
    VIRTUAL_IRQ_MAX_BASE = (uint32_t)(VIRTUAL_BLOCK_COUNT * VIRTUAL_IRQ_BLOCK_SIZE)
} virtual_irq_base_t;
 ...
 * @brief 判断当前是否在中断上下文
 */
COMPAT_STATIC_INLINE bool bottom_half_in_isr(void)
{
