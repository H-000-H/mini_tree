...
#define MQTT_MESSAGE_MAX_LENGTH   0x0FFFFFFF  /**< MQTT消息最大长度 */
/**
...
static bool mqtt_send_raw(struct mqtt_client_context *ctx,uint8_t *data, uint16_t len) 
{
    ...
    fifo_uni_read_block(&ctx->tx_fifo, data, len);
...
int mqtt_client_init(struct mqtt_client_context *ctx) 
{
    ...
    fifo_uni_init(&ctx->tx_fifo,ctx->tx_buffer,MQTT_BYTE_SIZE,MQTT_TX_BUFFER_SIZE);
    fifo_uni_init(&ctx->rx_fifo,ctx->rx_buffer,MQTT_BYTE_SIZE,MQTT_RX_BUFFER_SIZE);
    return ERR_OK;
}
