...
#define MQTT_CLIENT_ID "mini_node"
     ...
     * @return int 处理结果
     */
    typedef int (*mqtt_message_callback_t)(const char* topic, uint16_t topic_length,
    ...
    struct mqtt_client_context
    {
        ...
        TransportInterface_t
            transport_interface; /**< 传输接口 (指向下方 network_context 与胶水函数) */
     ...
     */
    int mqtt_client_publish(struct mqtt_client_context* context, const char* topic,
                            const void* payload, uint16_t length, uint8_t qos, bool retain);
